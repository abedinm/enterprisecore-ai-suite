...
# Per-token USD prices (rough public defaults — user can adjust via settings)
DEFAULT_PRICING = {
...
def available_providers() -> dict[str, bool]:
    return {
        ...
        "ollama": True,  # always available; will fail at call if Ollama isn't running
    }
...
def call_anthropic(messages: list[AiMessage], *, model: str | None = None,
                   max_tokens: int = 1024, temperature: float = 0.7) -> AiResponse:
...
def call_ollama(messages: list[AiMessage], *, model: str | None = None,
    ...
    return AiResponse(
        ...
        cost_usd=Decimal("0"),  # Ollama is local — no per-token cost
        latency_ms=latency_ms,
    )
...
def call(messages: list[AiMessage], *, provider: str | None = None, model: str | None = None,
         ...
         user_id: str | None = None) -> AiResponse:
    ...
    if not available.get(chosen):
        # Auto-fall back to the first available provider, or Ollama
    ...
    try:
        if chosen == "anthropic":
            result = call_anthropic(messages, model=model, max_tokens=max_tokens, temperature=temperature)
        elif chosen == "openai":
            result = call_openai(messages, model=model, max_tokens=max_tokens, temperature=temperature)
        else:
            result = call_ollama(messages, model=model, max_tokens=max_tokens, temperature=temperature)
...
def analyze_sentiment(text: str, *, db: Session | None = None,
    ...
    out = smart_text(prompt, feature="sentiment", db=db, user_id=user_id, max_tokens=200,
...
def summarise_meeting(transcript: str, *, db: Session | None = None,
    ...
    out = smart_text(prompt, feature="meeting_summary", db=db, user_id=user_id,
    ...
    try:
        ...
        return {"summary": out, "key_decisions": [], "action_items": []}


def explain_regex(pattern: str, *, db: Session | None = None, user_id: str | None = None) -> str:
    return smart_text(
