...
class Announcement(IdMixin, TenantMixin, TimestampMixin, Base):
    title: Mapped[str] = mapped_column(String(180), index=True)
    body: Mapped[str] = mapped_column(Text)
...
class CalendarEvent(IdMixin, TenantMixin, TimestampMixin, Base):
    title: Mapped[str] = mapped_column(String(180), index=True)
...
class SharedNote(IdMixin, TenantMixin, TimestampMixin, Base):
    title: Mapped[str] = mapped_column(String(180), index=True)
    body: Mapped[str] = mapped_column(Text, default="")
    owner_id: Mapped[str | None] = mapped_column(ForeignKey("users.id", ondelete="SET NULL"))


class Poll(IdMixin, TenantMixin, TimestampMixin, Base):
    question: Mapped[str] = mapped_column(String(255))
    closes_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))


class PollOption(IdMixin, TenantMixin, TimestampMixin, Base):
    poll_id: Mapped[str] = mapped_column(ForeignKey("polls.id", ondelete="CASCADE"))
    label: Mapped[str] = mapped_column(String(180))


class PollVote(IdMixin, TenantMixin, TimestampMixin, Base):
...
class WikiPage(IdMixin, TenantMixin, TimestampMixin, Base):
    ...
    __table_args__ = (UniqueConstraint("tenant_id", "slug", name="uq_wiki_pages_tenant_slug"),)
