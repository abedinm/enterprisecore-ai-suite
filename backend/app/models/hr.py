...
class Employee(IdMixin, TimestampMixin, Base):
    ...
    employee_code: Mapped[str] = mapped_column(String(60), unique=True, index=True)
...
class PerformanceReview(IdMixin, TimestampMixin, Base):
    employee_id: Mapped[str] = mapped_column(ForeignKey("employees.id", ondelete="CASCADE"), index=True)
...
class JobOpening(IdMixin, TimestampMixin, Base):
...
class Candidate(IdMixin, TimestampMixin, Base):
...
class OnboardingTask(IdMixin, TimestampMixin, Base):
...
class TrainingRecord(IdMixin, TimestampMixin, Base):
    employee_id: Mapped[str] = mapped_column(ForeignKey("employees.id", ondelete="CASCADE"), index=True)
...
class DisciplinaryRecord(IdMixin, TimestampMixin, Base):
    employee_id: Mapped[str] = mapped_column(ForeignKey("employees.id", ondelete="CASCADE"), index=True)
